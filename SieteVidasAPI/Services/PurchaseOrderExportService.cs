...
public class PurchaseOrderExportService : IPurchaseOrderExportService
{
    private const string Green = "#004C25";
    private const string HeaderColor = Green;
    ...
    public PurchaseOrderExportService()
    {
        var logoPath = Path.Combine(AppContext.BaseDirectory, "Assets", "logo_siete_vidas_completo.png");
    ...
    public byte[] CreateExcel(PurchaseOrderExportModel order)
    {
        ...
        sheet.Range("A1:N1").Merge().Style
            ...
            .Fill.SetBackgroundColor(XLColor.FromHtml(Green))
        ...
        headerRange.Style.Fill.SetBackgroundColor(XLColor.FromHtml(Green));
    ...
    public byte[] CreatePdf(PurchaseOrderExportModel order)
    {
        ...
        itemHeader.Shading.Color = Color.Parse(HeaderColor);
    ...
    private static void AddSectionTitle(Section section, string text, Unit? spaceBefore = null)
    {
        ...
        titleRow.Shading.Color = Color.Parse(HeaderColor);
    ...
    private static void AddTotalRow(Table table, string label, string value, bool highlight)
    {
        var row = table.AddRow();
        row.Cells[0].Borders.Visible = false;
        row.Cells[1].Shading.Color = highlight ? Color.Parse(HeaderColor) : Color.Parse("#F1F5F9");
        row.Cells[2].Shading.Color = highlight ? Color.Parse(HeaderColor) : Color.Parse("#F1F5F9");
    ...
    private static void AddExcelMetadata(IXLWorksheet sheet, int row, string label, object? value)
    {
        sheet.Cell(row, 1).Value = label;
        sheet.Cell(row, 1).Style.Font.SetBold().Font.SetFontColor(XLColor.FromHtml(Green));
        switch (value)
        {
            case DateTime date:
