...
const PurchaseOrderDetail = () => {
  ...
  const runAction = async (action, confirmation) => {
    if (confirmation && !await confirmDialog({ title: 'Confirmar acción', message: confirmation, confirmText: 'Continuar' })) return;
    try {
  ...
  return (
    <div className="animate-fade-in">
      ...
      {showItemModal && <div className="modal-overlay"><div className="modal-content purchase-selector-modal"><div className="purchase-modal-heading"><div><h3>Agregar artículos</h3><p>{materialsEnabled ? 'Productos terminados y materias primas disponibles.' : 'Productos terminados disponibles.'}</p></div><button type="button" className="purchase-modal-close" onClick={() => setShowItemModal(false)} aria-label="Cerrar"><X size={21} /></button></div><div className="purchase-search"><Search size={17} /><input className="input-field" autoFocus value={itemSearch} onChange={event => setItemSearch(event.target.value)} /></div>{materialsEnabled && <div className="purchase-filter-row">{[['Todos', 'Todos'], ['Producto', 'Productos'], ['MateriaPrima', 'Materias primas']].map(([value, label]) => <button key={value} type="button" className={itemFilter === value ? 'is-active' : ''} onClick={() => setItemFilter(value)}>{label}</button>)}</div>}<div className="purchase-selector-list">{filteredItems.map(item => {
