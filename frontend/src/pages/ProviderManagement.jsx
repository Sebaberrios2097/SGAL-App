...
const ProviderManagement = () => {
  ...
  const toggleStatus = async provider => {
    if (provider.activo && !await confirmDialog({ title: 'Desactivar proveedor', message: `“${provider.nombreProveedor}” ya no podrá seleccionarse en nuevas órdenes.`, confirmText: 'Desactivar', tone: 'danger' })) return;
    setSaving(true);
    setError('');
    setMessage('');
    try {
  ...
  return (
    <div className="animate-fade-in">
      ...
      {showModal && <div className="modal-overlay"><div className="modal-content provider-modal"><div className="purchase-modal-heading"><div><h3>{editing ? 'Editar proveedor' : 'Nuevo proveedor'}</h3><p>Estos datos se mostrarán en la orden de compra.</p></div><button type="button" className="purchase-modal-close" onClick={() => setShowModal(false)} aria-label="Cerrar"><X size={21} /></button></div><form onSubmit={saveProvider}>{error && <div className="provider-modal-error"><AlertCircle size={15} /> {error}</div>}<div className="provider-form-grid"><label className="input-group"><span className="input-label">Nombre del proveedor *</span><input className="input-field" autoFocus required maxLength="150" value={form.nombreProveedor} onChange={event => setForm(current => ({ ...current, nombreProveedor: event.target.value }))} /></label><label className="input-group"><span className="input-label">RUT</span><input className="input-field" maxLength="20" value={form.rut} onChange={event => setForm(current => ({ ...current, rut: event.target.value }))} /></label><label className="input-group"><span className="input-label">Persona de contacto</span><input className="input-field" maxLength="150" value={form.nombreContacto} onChange={event => setForm(current => ({ ...current, nombreContacto: event.target.value }))} /></label><label className="input-group"><span className="input-label">Teléfono</span><input className="input-field" type="tel" maxLength="30" value={form.telefono} onChange={event => setForm(current => ({ ...current, telefono: event.target.value }))} /></label><label className="input-group provider-form-wide"><span className="input-label">Correo</span><input className="input-field" type="email" maxLength="150" value={form.correo} onChange={event => setForm(current => ({ ...current, correo: event.target.value }))} /></label><label className="input-group provider-form-wide"><span className="input-label">Dirección</span><input className="input-field" maxLength="250" value={form.direccion} onChange={event => setForm(current => ({ ...current, direccion: event.target.value }))} /></label><label className="input-group"><span className="input-label">Comuna</span><input className="input-field" maxLength="100" value={form.comuna} onChange={event => setForm(current => ({ ...current, comuna: event.target.value }))} /></label><label className="input-group"><span className="input-label">Ciudad</span><input className="input-field" maxLength="100" value={form.ciudad} onChange={event => setForm(current => ({ ...current, ciudad: event.target.value }))} /></label></div><div className="purchase-modal-footer"><button type="button" className="btn btn-secondary" onClick={() => setShowModal(false)}>Cancelar</button><button type="submit" className="btn btn-primary" disabled={saving}>{saving ? 'Guardando…' : editing ? 'Guardar cambios' : 'Registrar proveedor'}</button></div></form></div></div>}
    </div>
  );
};
